                  ...
                  return (
                    <div key={pos.id} className={`bg-dark-700 rounded-lg p-4 ${pos.is_live ? 'border-l-4 border-green-500' : ''}`}>
                      <div className="flex justify-between items-start mb-3">
                        <div className="cursor-pointer" onClick={() => setSelectedPosition({ symbol: pos.symbol, name: pos.name, mode: 'trader' })}>
                          <div className="font-semibold text-white flex items-center gap-2 hover:text-emerald-400">
                            {pos.symbol}
                    ...
                    {displayedPositions.slice().sort((a, b) => (b.is_live ? 1 : 0) - (a.is_live ? 1 : 0) || (b.total_return_pct || 0) - (a.total_return_pct || 0)).map((pos) => {
                      ...
                      return (
                        <tr key={pos.id} className={`border-b border-dark-700/50 last:border-0 ${pos.is_live ? 'bg-green-500/5' : ''}`}>
                          <td className="py-3 px-4 cursor-pointer" onClick={() => setSelectedPosition({ symbol: pos.symbol, name: pos.name, mode: 'trader' })}>
                            <div className="flex items-center gap-2">
                              <div>
                                <div className="font-medium text-white flex items-center gap-2 hover:text-emerald-400">
                                  {pos.symbol}
                                  <BlockedBadge symbol={pos.symbol} blockedStocks={blockedStocks} />
                                  {pos.is_live && (
                                    <span className="px-2 py-0.5 bg-green-500 text-white text-[10px] font-bold rounded flex items-center gap-1">
